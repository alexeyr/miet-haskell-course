...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Для Haskell две основных библиотеки: \href{http://hackage.haskell.org/package/QuickCheck}{QuickCheck} и \href{https://github.com/hedgehogqa/haskell-hedgehog/}{Hedgehog} (есть ещё более новая \hackage{falsify}). Мы используем первую для простоты кода.
          \begin{haskell}
    ...
    \item Верно ли, что первый элемент списка всегда равен последнему элементу развёрнутого?
          ...
          \begin{ghcitiny}
            *** Failed! Exception: 'Prelude.head: empty list' (after 1 test):
            []
          \end{ghcitiny}
          (с GHC 9.8 уже само использование \haskinline|head| даёт предупреждение \ghcinline|x-partial|)
...
\begin{frame}[fragile]
  \frametitle{Почему деревья маленькие?}
  \begin{itemize}
    \item В первом варианте \haskinline|treeOf| каждый узел с вероятностью $1/2$ "--- лист, иначе у него два потомка.
    \item Это ветвящийся процесс (Гальтона-Ватсона) со средним числом потомков $2 \cdot 1/2 = 1$ "--- \emph{критический} случай.
          \pause
    \item Дерево конечно с вероятностью 1, но его \emph{ожидаемый} размер бесконечен: почти всегда оно крошечное, а изредка генерация не завершается за разумное время.
    \item Отсюда и параметр размера в QuickCheck (\haskinline|getSize|, \haskinline|resize|, \haskinline|sized|): размер должен убывать при рекурсии.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Для пар (компоненты уменьшаются по очереди; вариант \haskinline![(x', y') | x' <- shrink x, y' <- shrink y]! плох: если \haskinline|x| не уменьшается, не уменьшится и пара):
          \begin{haskelltiny}
    ...
    \item Для деревьев:
          \begin{haskelltiny}
            instance Arbitrary a => Arbitrary (Tree a) where
              ...
              shrink (Node x l r) =
                [Empty, l, r] ++
                [Node x' l r | x' <- shrink x] ++
                [Node x l' r | l' <- shrink l] ++
                [Node x l r' | r' <- shrink r]
          \end{haskelltiny}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \href{https://well-typed.com/blog/2023/04/falsify/}{falsify: Hypothesis-inspired shrinking for Haskell} (2023).
