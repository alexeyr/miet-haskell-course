...
Нужную версию GHC `stack` скачает сам при первой сборке (это несколько сотен мегабайт), отдельно ставить GHC не нужно.
...
Рекомендуемый редактор: [Visual Studio Code](https://code.visualstudio.com/) с расширением [Haskell](https://marketplace.visualstudio.com/items?itemName=haskell.haskell), но можно использовать и другие. Расширению нужен Haskell Language Server, он тоже ставится через `ghcup`: `ghcup install hls`. Инструкция по настройке: [README расширения (на английском)](https://github.com/haskell/vscode-haskell#readme).
