...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Мы можем увидеть частично вычисленные выражения в GHCi с помощью команд \href{https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/ghci.html#ghci-cmd-:print}{\haskinline|:print|} и \href{https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/ghci.html#ghci-cmd-:sprint}{\haskinline|:sprint|}:
          \begin{haskell}
  ...
  \begin{itemize}
    ...
    \item Классический пример:
          \begin{haskell}
            ...
            (100 + ... + (1 + 0))!$\pause \rightsquigarrow $! 5050
          \end{haskell}
  \end{itemize}
\end{frame}
  ...
  \begin{itemize}
    ...
    \item И процесс вычисления:
          \begin{haskell}
            ...
            !$\rightsquigarrow\ \ldots\ \rightsquigarrow\ $!5050
          \end{haskell}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Можем определить вариант \haskinline|foldl| (в библиотеке он есть в \haskinline|Data.List|/\haskinline|Data.Foldable|, а с GHC 9.10 и в \haskinline|Prelude|):
          \begin{haskell}
            foldl' :: (a -> b -> a) -> a -> [b] -> a
            foldl' f a []     = a
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Это превращается в
          ...
          \end{haskell}
          т.е. \texttt{!} вычисляет значение при сопоставлении. Этого хватает: \haskinline|foldl'| приводит каждую пару к WHNF, а это вычисляет \haskinline|s| и \haskinline|l|.
    \item Правила перевода \href{https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/exts/strict.html#bang-patterns-informal}{довольно сложные}.
  \end{itemize}
\end{frame}
  ...
  \begin{itemize}
    \item Другой способ решить ту же проблему:
          \begin{haskellsmall*}{escapeinside=||}
            ...
            mean :: [Double] -> Double
            mean xs = s' / fromIntegral l'
            where SPair s' l' = foldl' step (SPair 0 0) xs
