\documentclass[11pt]{beamer}
\input{lecture_preamble.tex}

\title{Лекция 6: работа со списками}

\begin{document}
\begin{frame}[plain]
  \maketitle
\end{frame}

\begin{frame}[fragile]
  \frametitle{Списки в Haskell}
  \begin{itemize}
    \item Поговорим о списках подробнее.
    \item Это структура данных, которая постоянно встречается в программах.
    \item Но по поведению и характеристикам \emph{очень сильно} отличается от \haskinline|ArrayList| в Java, \haskinline|List<T>| в C\# и \haskinline|std::vector| в C++.
          \pause
    \item Списки в Haskell и множестве других языков, начиная с Lisp "--- односвязные.
          \pause
    \item То есть вместе с каждым элементом хранится ссылка на остальную часть списка.
    \item Вспомним определение:
          \begin{haskell}
            data [a] = [] | a : [a]
          \end{haskell}
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Основные операции над списками}
  \begin{itemize}
    \item \haskinline|(++) :: [a] -> [a] -> [a]| \pause"--- конкатенация.
    \item \haskinline[escapeinside=``]|(!!) :: [a] -> Int -> a| \pause"--- элемент по индексу.
    \item \haskinline|null :: [a] -> Bool| \pause"--- проверка на пустоту.
    \item Если для непустых списков нужны голова и хвост, используйте сопоставление с образцом вместо \haskinline|null|, \haskinline|head| и \haskinline|tail|.
    \item \haskinline|elem :: Eq a => a -> [a] -> Bool| \pause"--- проверка на вхождение элемента.
    \item \haskinline|and, or :: [Bool] -> Bool| \pause"--- все ли элементы \haskinline|True| (или есть ли такой).
          \pause
    \item У части из них тип на самом деле более общий.
          \pause
    \item Все основные в \href{https://hackage.haskell.org/package/base/docs/Data-List.html}{Data.List}. И ещё пакет \hackage{safe}.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Некоторые функции второго порядка над\\ списками}
  \begin{itemize}
    \item \haskinline|map :: (a -> b) -> [a] -> [b]| \pause"--- применяет функцию ко всем элементам.
    \item \haskinline|filter :: (a -> Bool) -> [a] -> [a]| \pause"--- выбирает элементы, удовлетворяющие условию.
    \item \haskinline|concatMap :: (a -> [b]) -> [a] -> [b]| \pause"--- применяет функцию ко всем элементам и конкатенирует результаты.
    \item \haskinline|all, any :: (a -> Bool) -> [a] -> Bool| \pause"--- все ли элементы удовлетворяют условию (или есть ли такой).
          \pause
    \item[]
    \item Упражнение: реализуем какие-нибудь из них (рекурсивно и друг через друга).
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Сложность функций над списками}
  \begin{itemize}
    \item Базовые операции "--- конструирование списков и их разбор через \haskinline|:|.
          \pause
    \item Поэтому \haskinline|length| (а значит, и \haskinline|length xs == 0| без оптимизации) проходит весь список целиком, а \haskinline|null| нет.
          \pause
    \item \haskinline[escapeinside=``]|xs !! n| "--- цена пропорциональна \pause\haskinline|n|.
          \pause
    \item Чем определяется цена \haskinline|xs ++ ys|? \pause Длиной \haskinline|xs|.
          \pause
    \item Добавление в конец списка \haskinline|xs ++ [y]| получается дорогим.
    \item В цикле может сделать линейный алгоритм квадратичным.
          \pause
    \item Вместо добавления в конец часто лучше добавлять в начало, а потом развернуть.
          \pause
    \item Всё это осложняется ленивостью, о ней позже.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Выделения списков (list comprehensions)}
  \begin{itemize}
    \item Есть удобный способ записи списков, похожий на выражения для множеств.
    \item[] \haskinline![x*x | x <- [1..5], even x] == !\pause \haskinline|[4, 16]|.
    \item Как \(\left\{x^2\,\middle|\,x \in \{1, \ldots, 5\},\,even(x)\right\}\).
    \item \haskinline!x <- [1..5]! "--- \emph{генератор}, \haskinline|even x| "--- \emph{фильтр}.
          \pause
    \item Тех и других может быть много.
    \item \haskinline!let y = x*x! среди них вводит переменную.
    \item Генераторы могут использовать предыдущие переменные:
          \begin{haskell}
            [(x, y) | x <- [1..5], y <- [1..x], odd (x - y)] == !\pause!
              [(2,1), (3,2), (4,1), (4,3), (5,2), (5,4)]
          \end{haskell}
    \item Эти выражения преобразуются в комбинации \haskinline|concatMap|, \haskinline|map| и \haskinline|filter|:
          \begin{haskell}
            [x*x | x <- [1..5], even x] == !\pause!
                map (\x -> x*x) (filter even [1..5])
          \end{haskell}
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Правая свёртка списков и структурная рекурсия}
  \begin{itemize}
    \item При \emph{структурной рекурсии} рекурсивный вызов делается на подтерме одного из аргументов функции (например, хвосте списка).
    \pause
    \item Есть универсальная функция для таких определений:
          \begin{haskell}
            foldr :: (a -> b -> b) -> b -> [a] -> b
            foldr f z [x1, x2, ..., xn] == 
              x1 `f` (x2 `f` ... (xn `f` z)...)
          \end{haskell}
          \begin{itemize}
            \item Настоящий тип более общий: \haskinline|Foldable t => ... -> t a -> b| (лекция 9).
          \end{itemize}
    \item Правая свёртка проходит по структуре списка и заменяет \haskinline{[]} на \haskinline{z} и \haskinline{(:)} на \haskinline{f}.
    \item Например, \haskinline|sum| получится, если взять \haskinline{f =!\pause! (+)} и \haskinline{z =!\pause! 0}.
    \item Упражнение: реализовать \haskinline|length|.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Правая свёртка: реализация}
  \begin{haskell}
    foldr :: (a -> b -> b) -> b -> [a] -> b
    foldr f z [] = !\pause!z
    foldr f z (x:xs) = !\pause!f x !\pause!(foldr f z xs)
  \end{haskell}
  \begin{itemize}
    \item Пустой список заменяем на \haskinline|z|.
    \item Для \haskinline|x:xs| сворачиваем хвост и передаём \haskinline|f| голову и результат свёртки хвоста.
    \item Та же схема работает для других типов данных: каждому конструктору соответствует своя функция.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Левая свёртка и хвостовая рекурсия}
  \begin{itemize}
    \item При \emph{хвостовой рекурсии} результат рекурсивного вызова сразу становится результатом текущего вызова. Например,
    \item \begin{haskellsmall}
            sum xs = sum' 0 xs where
              sum' acc [] = acc
              sum' acc (x:xs) = sum' !\pause!(acc + x) xs
          \end{haskellsmall}
    \item Универсальная функция для таких определений:
          \begin{haskellsmall}
            foldl :: (b -> a -> b) -> b -> [a] -> b
            foldl _f z [] = z
            foldl f z (x:xs) = foldl f !\pause!(f z x) xs
          \end{haskellsmall}
    \item Она проходит по списку слева и получается 
          \begin{haskellsmall}
            (...((z `f` x1) `f` x2) `f`...) `f` xn
          \end{haskellsmall}
    \item Хвостовая рекурсия компилируется в цикл и в других языках ФП очень важна; в Haskell её усложняет ленивость (лекция 8).
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Свёртки других типов данных}
  \begin{itemize}
    \item Сравним объявление списка с типом \haskinline|foldr|:
          \begin{haskellsmall}
            data [a] = a : [a]       | []
            foldr ::  (a -> b -> b) -> b -> [a] -> b
          \end{haskellsmall}
    \item Видим, что у нас по одному аргументу для каждого конструктора: каждый принимает аргументы типов полей этого конструктора (а если там \haskinline|[a]|, то \haskinline|b|) и возвращает \haskinline|b|.
    \item Последний аргумент "--- разбираемый список.
    \item Для \haskinline|Maybe|:
          \begin{haskellsmall}
            data Maybe a = Nothing | Just a
            foldMaybe ::   ?      -> ?        -> ?       -> b !\pause!
            foldMaybe ::   b      -> (a -> b) -> Maybe a -> b
          \end{haskellsmall}
          \pause
    \item Это в точности \haskinline|maybe| из \haskinline|Prelude|.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Свёртка деревьев}
  \begin{itemize}
    \item Для деревьев возьмём пример, где данные хранятся в листьях, но не в ветвях:
          \begin{haskellsmall}
            data Tree a = Leaf a | Node (Tree a) (Tree a)
            foldTree   :: ? ->        ?             -> Tree a -> b !\pause!
            foldTree   :: (a -> b) -> (b -> b -> b) -> Tree a -> b !\pause!

            sumTree :: Num a => Tree a -> a
            sumTree = foldTree!\pause! id (+)
          \end{haskellsmall}
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Свёртка деревьев: реализация и пример}
  \begin{haskellsmall}
    foldTree :: (a -> b) -> (b -> b -> b) -> Tree a -> b
    foldTree leaf node (Leaf x) = leaf x
    foldTree leaf node (Node left right) =
        node (foldTree leaf node left)
             (foldTree leaf node right)

    sumTree :: Num a => Tree a -> a
    sumTree = foldTree id (+)

    ghci> sumTree (Node (Leaf 2) (Node (Leaf 3) (Leaf 5)))
    10
  \end{haskellsmall}
  \begin{itemize}
    \item Заменяем \haskinline|Leaf| на \haskinline|id|, а \haskinline|Node| на \haskinline|(+)|: получаем \haskinline|2 + (3 + 5)|.
    \item Два рекурсивных поля узла дают два результата типа \haskinline|b|, которые объединяет функция \haskinline|node|.
  \end{itemize}
\end{frame}
\begin{frame}[fragile]
  \frametitle{Дополнительное чтение}
  \begin{itemize}
    \item \href{https://habr.com/ru/articles/667058/}{Применение обобщённой свёртки для обработки синтаксических деревьев}
    \item \href{https://people.cs.nott.ac.uk/pszgmh/fold.pdf}{A tutorial on the universality and expressiveness of fold}
    \item \href{https://wiki.haskell.org/wikiupload/1/14/TMR-Issue6.pdf}{Getting a Fix from the Right Fold} (реализация \haskinline{dropWhile} через \haskinline{foldr})
  \end{itemize} 
\end{frame}

\end{document}
