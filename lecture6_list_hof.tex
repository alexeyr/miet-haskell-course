    ...
    \item
          \begin{haskell}
            ...
            foldl f z [x1, x2, ..., xn] == 
              (...((z `f` x1) `f` x2) `f`...) `f` xn
          \end{haskell}
    \item Функции \haskinline|sum|, \haskinline|length| и т.д. можно реализовать через них (TODO)  % TODO
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Для \haskinline|Maybe|:
          ...
          \end{haskellsmall}
          \pause
    \item Это \haskinline|Data.Maybe.maybe| с обратным порядком аргументов. (TODO Bool и деревья)  % TODO
  \end{itemize}
\end{frame}
