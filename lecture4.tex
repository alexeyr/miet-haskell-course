    ...
    \item Ещё пример:\pause
          \begin{haskell}
            instance Eq Shape where
                ...
                Rectangle w h == Rectangle v k = w == v && h == k
                _ == _ = False
          \end{haskell}
          \only<3>{\item Рекурсивно ли это определение?}
    \item<4-> Это не рекурсия: \haskinline|==| в правой части относится к другому типу (\haskinline|Double|).
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item В \haskinline|rest@(y : _)| имя \haskinline|rest| обозначает весь список, подходящий под образец \haskinline|y : _|.
    \item Ограничение нужно и функциям, использующим \haskinline|==|:
          \begin{haskell}
            foo [] = True
            foo [x] = True
            foo (x : rest@(y : _)) = x == y && foo rest
          \end{haskell}
          Какой у неё тип (и смысл)? \pause
    \item \haskinline|foo :: Eq a => [a] -> Bool| \pause
    \item Все ли элементы равны? Подходящее имя "--- \haskinline|allEqual|.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item В C++ близкое понятие "--- концепции: до C++20 только в документации, теперь и в коде. \pause
    \item Часто сравнивают с интерфейсами Java/C\#, но это хуже: \pause у классов типов есть \enquote{статические} и бинарные методы. \pause
    \item Ближайший аналог "--- трейты (traits) в Rust.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Или прочитать из строк:
          ...
          \end{haskell}
          \pause
    \item Для нас то, что за многоточиями, неважно, но подробности описаны \href{https://hackage.haskell.org/package/base/docs/Prelude.html#g:22}{в документации}.\pause
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \end{haskell}
    \item \href{https://hackage.haskell.org/package/base/docs/Prelude.html#g:7}{Полная документация.}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Для разных структур из общей алгебры или линейной алгебры можно определить соответствующие им классы. В \haskinline|Prelude| есть
    \item полугруппы (ассоциативная операция):
          \begin{haskell}
            class Semigroup a where
                (<>) :: a -> a -> a
          \end{haskell}
    \item и моноиды (полугруппа с нейтральным элементом):
          \begin{haskell}
            class Semigroup a => Monoid a where
                mempty :: a
                mappend :: a -> a -> a -- = (<>), устарело
                mconcat :: [a] -> a    -- = foldr (<>) mempty
          \end{haskell}
    \item Так стало с GHC 8.4; в старом коде и книгах \haskinline|mappend| "--- основной метод, а \haskinline|Semigroup| нет вообще. \pause
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item В \haskinline|Ord| можно было бы оставить членом класса только \haskinline|compare|, а \haskinline|(<)| и остальные определить через него.
    \item Но у \haskinline|Double| есть \haskinline|NaN|, который не сравним ни с чем, а \haskinline|Ordering| не может этого выразить:
          \begin{haskell}
            ghci> nan = 0/0 :: Double
            ghci> compare nan nan
            GT
            ghci> nan > nan  -- определено напрямую
            False
          \end{haskell}
    \item Если бы \haskinline|(>)| определялось как \haskinline|compare x y == GT|, получилось бы \haskinline|True|.
    \item Поэтому все функции сравнения "--- члены \haskinline|Ord|, и экземпляр для \haskinline|Double| определяет каждую напрямую.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Почти все определения \haskinline|(==)| устроены одинаково:\pause
          ...
          \end{itemize}
    \item Почти, но не все: \pause например, для \haskinline|Data.Set| и \haskinline|Data.Map| (деревья с одними и теми же элементами могут иметь разную форму) или для типов с кэшированными полями.\pause
    ...
    \item \haskinline|deriving| также работает для \haskinline|Ord|, \haskinline|Show|, \haskinline|Read|, \haskinline|Enum|, \haskinline|Bounded| и \haskinline|Ix|.

    \item В GHC2021 выводятся также \haskinline|Functor|, \haskinline|Foldable| и \haskinline|Traversable| (лекция 9).
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Вывод \haskinline|Ord|, \haskinline|Enum| и \haskinline|Bounded|}
  \begin{haskell}
  ...
  \end{haskell}
  \begin{itemize}
    \item \haskinline|Ord| сравнивает сначала конструкторы в порядке объявления, затем поля слева направо (лексикографически).
    \item Стандартный вывод \haskinline|Enum| требует конструкторов без полей.
    \item \haskinline|Bounded| выводится для перечислений, а также для одного конструктора, если типы всех его полей имеют \haskinline|Bounded|.
  \end{itemize}
\end{frame}
\begin{frame}[fragile]
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \haskinline|GeneralizedNewtypeDeriving| (входит в \haskinline|GHC2021|) позволяет переносить многие экземпляры завёрнутого типа на \haskinline|newtype|.\pause
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Все перечислять не будем, они есть в \href{https://downloads.haskell.org/~ghc/latest/docs/html/users_guide/exts/instances.html}{User's Guide (Instance declarations and resolution)}.
    \item \haskinline|-XUndecidableInstances| и прагмы \haskinline|{-# OVERLAPPING #-}|/\haskinline|{-# INCOHERENT #-}| у отдельных экземпляров (расширения \haskinline|OverlappingInstances|/\haskinline|IncoherentInstances| устарели с GHC 7.10) опаснее, но в этом курсе в любом случае не понадобятся.
  \end{itemize}
\end{frame}
