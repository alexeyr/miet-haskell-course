...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Функции могут принимать на вход функции.
    ...
    \item В других языках \emph{функциями высших порядков (ФВП)} называют функции, аргументы или результаты которых тоже функции.\pause
    \item Функция второго порядка принимает или возвращает обычную функцию (первого порядка) и так далее.\pause
    \item В этой лекции ФВП называем только функции, принимающие функции. Почему "--- увидим позже.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Но есть специальный синтаксис \haskinline|(арг оп)| и \haskinline|(оп арг)|:
          ...
          \end{haskell}
          \pause
    \item Ловушка: \haskinline|(- 2)| "--- это число $-2$, а не сечение. Для вычитания есть \haskinline|subtract 2|.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Конструкторы кортежей тоже можно применять частично (\haskinline|TupleSections|, входит в GHC2021):
          \begin{haskell}
            ghci> (, "I", , "Love", True) 1 False !\pause!
            (1,"I",False,"Love",True)
            ghci> :t (, "I", , "Love", True) !\pause!
            (, "I", , "Love", True)
              :: t1 -> t2 -> (t1, String, t2, String, Bool)
          \end{haskell}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Рассмотрим два определения
          \begin{haskell}
            foo' x = foo x
            -- или foo' = \textbackslash x -> foo x 
          \end{haskell}
    \item Применение обоих определений к одному аргументу задаёт одно и то же вычисление.\pause
    \item Поэтому обычно можно сократить аргумент:
          \begin{haskell}
            foo' = foo
          \end{haskell}
\pause
    \item Оговорка: удаление лямбды может изменить поведение при использовании \haskinline|seq| (подробнее в лекции о ленивости).
    \item Можно сокращать и несколько аргументов в конце:
          \begin{haskell}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item И когда само определение такой формы не имеет, но может быть к ней преобразовано
          \begin{haskell}
            root4 :: Floating a => a -> a
            root4 x = sqrt (sqrt x)
          \end{haskell}
          можно переписать (сохраняя сигнатуру) как
          \begin{haskell}
            root4 x = (sqrt . sqrt) x
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Композиция и другие операции позволяют определять функции без явного указания аргументов.
    \item Это называется \emph{бесточечным стилем} (переменные рассматриваются как точки в пространстве значений).\pause
    \item Многие выражения имеют бесточечный эквивалент.
    \item На \href{https://wiki.haskell.org/Pointfree}{Haskell Wiki} есть инструменты для перевода между стилями.
    \item Опять же, не перестарайтесь:
          ...
          \end{haskell}
          \pause
    \item Яснее выразить тот же смысл через \haskinline|on|:
          \begin{haskell}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Если в выражении (а не в образце) использовать дыру \haskinline|_| (или \haskinline|_название|; дыр может быть несколько), то увидим ожидаемый в этом месте тип.
    \item
          \begin{haskell}
            foo :: (a -> b, b -> c) -> (a -> c)
            foo (f, g) = \x -> g _
          ...
          \begin{ghcismall}
            ...
            • Relevant bindings include
                x :: a, g :: b -> c, f :: a -> b, ...
          \end{ghcismall}
    \item \emph{Rigid} (жёсткая) переменная типа "--- из сигнатуры: тип выбирает вызывающий, и внутри определения её нельзя заменить конкретным. Та же фраза будет в ошибке, если вернуть \haskinline|Int| вместо \haskinline|a|.
  \end{itemize}
\end{frame}
