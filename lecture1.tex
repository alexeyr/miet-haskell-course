...
\begin{frame}
  \frametitle{От C к Haskell: что добавляется}
  \centering
  \begin{tikzpicture}[
      x=1cm, y=1cm,
      lang/.style={draw=mDarkTeal, thick, rounded corners=2pt, fill=mDarkTeal!8,
          minimum width=17mm, minimum height=6.5mm, font=\small\bfseries},
      feat/.style={draw=mLightBrown!60, fill=mLightBrown!10, rounded corners=2pt,
          font=\scriptsize, align=center, inner sep=3pt},
      arr/.style={-stealth, thick, black!55}]
    \node[lang] (c)     at (0, 5.0)     {C};
    \node[feat] (f1)    at (0, 4.1)     {+ функции первого класса (замыкания)};
    \node[lang] (py)    at (-2.6, 3.2)  {Python};
    \node[lang] (java)  at (2.6, 3.2)   {Java};
    \node[feat] (f2)    at (0, 2.1)     {+ выражения вместо инструкций\\ + неизменяемость по умолчанию\\ + вывод типов};
    \node[lang] (ocaml) at (-2.6, 1.0)  {OCaml};
    \node[lang] (scala) at (2.6, 1.0)   {Scala};
    \node[feat] (f3)    at (0, 0.0)     {+ ленивость\\ + чистота: эффекты видны в типах};
    \node[lang, draw=mLightBrown, fill=mLightBrown!20] (hs) at (0, -1.0) {Haskell};
    \draw[arr] (c) -- (f1);
    \draw[arr] (f1) -- (py);
    \draw[arr] (f1) -- (java);
    \draw[arr] (py) -- (f2.north);
    \draw[arr] (java) -- (f2.north);
    \draw[arr] (f2.south) -- (ocaml);
    \draw[arr] (f2.south) -- (scala);
    \draw[arr] (ocaml) -- (f3.north);
    \draw[arr] (scala) -- (f3.north);
    \draw[arr] (f3) -- (hs);
  \end{tikzpicture}

  \footnotesize Упрощённо: у каждой стрелки есть оговорки (Java~8+, Scala~3, аннотации типов в Python).
\end{frame}
  ...
  \begin{itemize}
    \item Значения лучше переменных.
          \begin{itemize}
            ...
            \item Значения не изменяются после создания.
          \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Purescript, Elm: для веба
    \item Семейство ML: OCaml, SML, F\#
          \pause
    \item \textbf{Haskell} (нижний узел схемы \enquote{От C к Haskell}):
          \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Пример:
          ...
          \begin{haskell}
            ...
            True :: Bool
          \end{haskell}
    \item Попробовать онлайн: \url{https://play.haskell.org/}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item По коду \ghcinline|[GHC-39999]| на \url{https://errors.haskell.org} есть объяснение ошибки и примеры.
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Применение функций}
  \begin{itemize}
...
\begin{frame}[fragile]
  \frametitle{Приоритет применения функций}
  \begin{itemize}
    \item Применение функции связывает сильнее любого инфиксного оператора:
          \begin{haskell}
            ghci> abs (-2) + 3 !\pause!
            5
            ghci> abs (-2 + 3) !\pause!
            1
            ghci> max 2 3 + 4 !\pause!
            7
          \end{haskell}
    \item \haskinline|abs| возвращает модуль числа; \haskinline|abs (-2) + 3| читается как \haskinline|(abs (-2)) + 3|.
    \pause
    \item Без скобок \haskinline|abs -2 + 3| читается как \haskinline|(abs - 2) + 3|: минус означает вычитание.
    \item Получится ошибка типов: мы пытаемся вычесть число из функции \haskinline|abs|.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Определение функции почти такое же:
          ...
          \end{haskell}
    \item \haskinline|=| используется в определениях, \haskinline|==| "--- для проверки равенства.
  ...
  \begin{itemize}
    ...
    \item Целые числа:
          \begin{itemize}
            ...
            \item \haskinline|Int|: машинные\footnote{по стандарту минимум 30 бит, но в GHC именно 32 или 64 бита}, \haskinline|Word|: машинные без знака;
            \item \haskinline|Int8|\ldots\haskinline|Int64| (модуль \haskinline|Data.Int|) и \haskinline|Word8|\ldots\haskinline|Word64| (\haskinline|Data.Word|): фиксированного размера, со знаком и без.
          \end{itemize}
    \item \haskinline|Float| и \haskinline|Double|: 32- и 64-битные числа с плавающей точкой, по стандарту IEEE-754.
    \item \haskinline|Char|: символы Unicode.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item А может выводиться из контекста:
          \begin{haskell}
            ghci> :t fromIntegral x / (4 :: Double)
            fromIntegral x / (4 :: Double) :: Double
          \end{haskell}
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Приведение числовых типов (справочно)}
  \begin{itemize}
    \item \haskinline|toInteger| переводит любой целочисленный тип \\в \haskinline|Integer|.
    \item \haskinline|fromInteger| переводит \haskinline|Integer| в любой числовой тип, в том числе \haskinline|Double|.
    \item При переводе в целые фиксированной разрядности значение может переполниться. Например, в GHC:
          \begin{haskell}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \haskinline|/| "--- деление \emph{дробных} чисел.
          \begin{itemize}
            \item Использование \haskinline|/| для значений целочисленных типов (\haskinline|Int|, \haskinline|Integer|) даст ошибку \ghcinline|No instance... arising from a use of ‘/’|. Нужно сначала использовать \haskinline|fromIntegral|.
            \item \haskinline|2 / 4| работает, но \haskinline|(2 :: Integer) / 4| даёт ошибку типов.
          \end{itemize}
...
\begin{frame}[fragile]
  \frametitle{Проверьте себя}
  \begin{enumerate}
    \item Предскажите результаты, затем проверьте их в GHCi:
          \begin{haskell}
            square 3 + 2
            square (3 + 2)
          \end{haskell}
    \item Определите функцию \haskinline|root4|, которая вычисляет корень 4-й степени из числа (используйте \haskinline|sqrt|). Проверьте \haskinline|root4 16|.
    \item Пусть \haskinline|x = 3 :: Integer|. Вычислите половину \haskinline|x| как \haskinline|Double| с помощью \haskinline|fromIntegral|.
  \end{enumerate}
\end{frame}
%\begin{frame}
