...
\begin{frame}[fragile]
  ...
  \begin{itemize}[<+->]
    ...
    \item \haskinline|sequence :: Monad m => [m a] -> m [a]|. В библиотеке более общий вариант: \haskinline|sequenceA| из \haskinline|Traversable|.
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item В библиотеках \hackage{transformers} и \hackage{mtl} (модуль \haskinline|Control.Monad.State|) \haskinline|State s| определён как \haskinline|StateT s Identity| (трансформеры "--- в лекции 14).
    ...
    \item Типичная ошибка: написать \haskinline|State (\s -> ...)| после \haskinline|import Control.Monad.State| и получить \ghcinline|Data constructor not in scope: State|.
  \end{itemize}
\end{frame}
