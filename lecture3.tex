...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Создание значения:
          \begin{haskell}
ghci> unitCircle = Circle 1
\end{haskell}
    \item Образцы: \haskinline|Circle r| и \haskinline|Rectangle w h| (\haskinline|r|, \haskinline|w|, \haskinline|h| "--- образцы для \haskinline|Double|).
    ...
    \item Но не обязательно:
          \begin{haskell}
            isUnitCircle (Circle 1) = True

            isUnitCircle _ = False
          \end{haskell}
          \pause
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item Возможность отсутствия значения отражена в типе: \haskinline|Maybe a|.
    \item Документация объясняет, в каких случаях возвращается \haskinline|Nothing|.
  ...
  \begin{itemize}
    \item Мы могли бы определить типы
          \begin{haskell}
            ...
            ...
          \end{haskell}
          Слева \haskinline|Pair| "--- имя типа, справа "--- конструктора данных. Параметры \haskinline|a| и \haskinline|b| слева объявляются, справа используются как типы полей.
    ...
    \item Максимальный размер в GHC "--- 64 (стандарт языка требует поддержки только до 15).
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item В стандартной библиотеке:
          ...
          \end{haskell}
    \item Синоним документирует назначение, но не создаёт отдельный тип. О недостатках представления \haskinline|String| поговорим позже.
          \pause
    \item Для \haskinline|FilePath| дело в том, что осмысленные операции над путями не те, что над строками:
          \begin{haskell}
            ...
            ghci> :{
            ghci| isPrefixOf ("C:\\Program Files" :: FilePath)
            ghci|            ("C:\\Program Files (x86)" :: FilePath)
            ghci| :}
            True
          \end{haskell}
  \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Функции над рекурсивными типами часто тоже рекурсивны (или вызывают рекурсивные).
          ...
          \end{haskell}
    \item Рекурсивный вызов получает более короткий хвост; на пустом списке рекурсия заканчивается:
          \begin{haskell}
            length [10, 20]
              = length [20] + 1
              = (length [] + 1) + 1
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item Несколько полей у конструктора соответствуют декартову произведению множеств, а несколько конструкторов\only<1>{ "--- чему?}\only<2->{ "--- дизъюнктному объединению $A_1 \sqcup A_2 = \{1\} \times A_1 \cup \{2\} \times A_2$.}
    \onslide<2->{\item Сравните помеченное объединение с непомеченным \haskinline|union| в C/C++ и с \haskinline|std::variant|.}
  \end{itemize}
  \onslide<3->{
    \centering\small
    \begin{tabular}{llll}
      \toprule
      Тип & Множества & Логика & Алгебра \\
      \midrule
      \haskinline|(a, b)| & $A \times B$ & $A \wedge B$ & $a \cdot b$ \\
      \haskinline|Either a b| & $A \sqcup B$ & $A \vee B$ & $a + b$ \\
      \haskinline|a -> b| & $B^A$ & $A \to B$ & $b^a$ \\
      \haskinline|()| & $\{\ast\}$ & истина & $1$ \\
      \haskinline|Void| & $\varnothing$ & ложь & $0$ \\
      \bottomrule
    \end{tabular}
    \par
  }
  \onslide<4->{
    \begin{itemize}
      \item Законы алгебры верны с точностью до изоморфизма: $c^{a+b} = c^a \cdot c^b$, т.е. \haskinline|Either a b -> c| $\cong$ \haskinline|(a -> c, b -> c)|.
    \end{itemize}
  }
\end{frame}
  ...
  \begin{itemize}
    \item
          \begin{haskellsmall}
            ...
            ghci> x' !\pause!
            0.0 !\pause!
            ghci> :set -XOverloadedRecordDot -XRecordWildCards
            ghci> aPoint.xCoord
            0.0 !\pause!
...
\begin{frame}[fragile]
  \frametitle{Переменные типа: область видимости}
  \begin{itemize}
    \item Дополнительный материал. По умолчанию область видимости переменной типа "--- сигнатура, где она появилась. Например, в
          \begin{haskell}
            moveToEnd :: [a] -> [a]
            moveToEnd (x:xs) = xs ++ [x :: a]
          \end{haskell}
          \pause
          две переменные \haskinline|a| "--- разные и читаются как
          \begin{haskell}
            moveToEnd :: forall a. [a] -> [a]
            moveToEnd (x:xs) = xs ++ [x :: forall a. a]
          \end{haskell}
          \pause
          (так что функция не скомпилируется).
          \pause
    \item С \haskinline|ScopedTypeVariables| (входит в \haskinline|GHC2021|) можно написать так:
          \begin{haskell}
            moveToEnd :: forall a. [a] -> [a]
            moveToEnd (x:xs) = xs ++ [x :: a]
          \end{haskell}
    \item Точные правила можно найти в \href{https://downloads.haskell.org/~ghc/latest/docs/users_guide/exts/scoped_type_variables.html}{Lexically scoped type variables}.
  \end{itemize}
\end{frame}
\end{document}
