...
\begin{frame}[fragile]
  \frametitle{А как же классы типов?}
  \begin{itemize}
    \item Мы выводили типы без ограничений, но \haskinline|1 + x| требует \haskinline|Num|.
          \pause
    \item Расширение системы Хиндли-Милнера "--- \emph{квалифицированные типы}: кроме подстановки $\phi$ накапливается набор ограничений; при обобщении в \haskinline|let| они попадают в контекст: \haskinline|Num a => a -> a|.
          \pause
    \item Реализация "--- передача \enquote{словарей}: \haskinline|Num a => a -> a| компилируется примерно в \haskinline|NumDict a -> a -> a|.
          \pause
  \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Неоднозначные типы и умолчания}
  \begin{itemize}
    \item Проблема: если переменная типа осталась \emph{только} в ограничениях, тип неоднозначен. Какой тип у \haskinline|read s| в \haskinline|show (read s)|?
          \pause
    \item Для числовых классов есть правило умолчания (defaulting): пробуются \haskinline|Integer|, затем \haskinline|Double|. Отсюда предупреждение \ghcinline|Defaulting the following constraints to type 'Integer'| и то, что \haskinline|2^64| в GHCi "--- \haskinline|Integer|, а не \haskinline|Int|.
          \pause
    \item В GHCi правила расширены (\haskinline|ExtendedDefaultRules|, для прочих классов подставляется \haskinline|()|), поэтому \haskinline|print []| там работает, а в файле "--- ошибка \ghcinline|Ambiguous type variable|.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    ...
    \item \href{https://github.com/mgrabmueller/AlgorithmW}{Algorithm W Step by Step}
